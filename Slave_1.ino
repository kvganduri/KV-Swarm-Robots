// --- START: Rover 1 ESP32 Code (V1.1 - Added m/h sequence) ---
...
// --- PID Tuning Parameters --- (Use values tuned for Rover 1)
// !!! REPLACE with your tuned values for ROVER 1 !!!
double Kp_pid = 139.5; // Example from previous tuning
double Ki_pid = 7.0;   // Example from previous tuning
double Kd_pid = 1.0;   // Example from previous tuning
...
// *** ADDED States for 'm' sequence and its 'h' return ***
enum RoverState {
    IDLE,
    ...
    SHAPE_N_R1_LEG1,        // Shape 'n' (Rover 1) states
    SHAPE_N_R1_TURN1,
    SHAPE_N_R1_LEG2,
    SHAPE_N_R1_WAIT_HOME,   // Wait state for 'h' after 'n'

    SHAPE_H_N_R1_TURN1,     // Return sequence after 'n'
    SHAPE_H_N_R1_LEG1,
    SHAPE_H_N_R1_TURN2,
    SHAPE_H_N_R1_LEG2,

    SHAPE_M_R1_LEG1,        // Shape 'm' (Rover 1) states
    SHAPE_M_R1_TURN1,
    SHAPE_M_R1_LEG2,
    SHAPE_M_R1_WAIT_HOME,   // Wait state for 'h' after 'm'

    SHAPE_H_M_R1_TURN1,     // Return sequence after 'm'
    SHAPE_H_M_R1_LEG1,
    SHAPE_H_M_R1_TURN2,
    SHAPE_H_M_R1_LEG2
};
...
void setup() {
    Serial.begin(115200);
    Serial.println("Rover 1 Setup (V1.1 - Added m/h sequence)..."); // Version Bump
...
void loop() {
    ...
    switch (currentState) {
        ...
        case SHAPE_M_R1_LEG1: // Added M leg 1
        case SHAPE_M_R1_LEG2: // Added M leg 2
        case SHAPE_H_N_R1_LEG1:
        case SHAPE_H_N_R1_LEG2:
        case SHAPE_H_M_R1_LEG1: // Added M home leg 1
        case SHAPE_H_M_R1_LEG2: { // Added M home leg 2
            ...
            if (targetReached) {
                ...
                else if (currentState == SHAPE_N_R1_LEG1){ Serial.println("Shape N Leg 1 (105cm) complete. Starting Turn 1 (R90)..."); startYawForTurn = currentYaw; targetTurnAngle = 90.0; setSideSpeed(LEFT_SIDE, -1, TURN_SPEED); setSideSpeed(RIGHT_SIDE, 1, TURN_SPEED); currentState = SHAPE_N_R1_TURN1; }
                ...
                else if (currentState == SHAPE_H_N_R1_LEG1){ Serial.println("Shape Home (N) Leg 1 (30cm) complete. Starting Turn 2 (L90)..."); startYawForTurn = currentYaw; targetTurnAngle = 90.0; setSideSpeed(LEFT_SIDE, 1, TURN_SPEED); setSideSpeed(RIGHT_SIDE, -1, TURN_SPEED); currentState = SHAPE_H_N_R1_TURN2; }
                ...
                else if (currentState == SHAPE_M_R1_LEG1){ Serial.println("Shape M Leg 1 (110cm) complete. Starting Turn 1 (R90)..."); startYawForTurn = currentYaw; targetTurnAngle = 90.0; setSideSpeed(LEFT_SIDE, -1, TURN_SPEED); setSideSpeed(RIGHT_SIDE, 1, TURN_SPEED); currentState = SHAPE_M_R1_TURN1; }
                else if (currentState == SHAPE_M_R1_LEG2){ Serial.println("Shape M Leg 2 (65cm) complete. Waiting for Home..."); currentState = SHAPE_M_R1_WAIT_HOME; } // Corrected distance comment
                else if (currentState == SHAPE_H_M_R1_LEG1){ Serial.println("Shape Home (M) Leg 1 (65cm) complete. Starting Turn 2 (L90)..."); startYawForTurn = currentYaw; targetTurnAngle = 90.0; setSideSpeed(LEFT_SIDE, 1, TURN_SPEED); setSideSpeed(RIGHT_SIDE, -1, TURN_SPEED); currentState = SHAPE_H_M_R1_TURN2; }
                else if (currentState == SHAPE_H_M_R1_LEG2){ Serial.println("Shape Home (M) Leg 2 (110cm) complete. Sequence Finished."); lastShapeCommand = '\0'; currentState = IDLE; }
        ...
        case SHAPE_N_R1_TURN1: { float angleTurned = abs(calculateAngleDifference(startYawForTurn, currentYaw)); if (angleTurned >= targetTurnAngle - TURN_ANGLE_THRESHOLD) { Serial.println("Shape N Turn 1 (R90) complete. Starting Leg 2 (30cm)..."); stopMotors(); prepareForwardMove(30.0); currentState = SHAPE_N_R1_LEG2; } break; }
        case SHAPE_N_R1_WAIT_HOME: { /* Do nothing, wait for 'h' in OnDataRecv */ break; }

        // --- Shape M (Rover 1) Specific States ---
        case SHAPE_M_R1_TURN1: { float angleTurned = abs(calculateAngleDifference(startYawForTurn, currentYaw)); if (angleTurned >= targetTurnAngle - TURN_ANGLE_THRESHOLD) { Serial.println("Shape M Turn 1 (R90) complete. Starting Leg 2 (65cm)..."); stopMotors(); prepareForwardMove(45.0); currentState = SHAPE_M_R1_LEG2; } break; } // Corrected distance
        case SHAPE_M_R1_WAIT_HOME: { /* Do nothing, wait for 'h' in OnDataRecv */ break; }

        // --- Shape Home Return (Rover 1 - after N) States ---
        case SHAPE_H_N_R1_TURN1: { float angleTurned = abs(calculateAngleDifference(startYawForTurn, currentYaw)); if (angleTurned >= targetTurnAngle - (TURN_ANGLE_THRESHOLD * 1.5)) { Serial.println("Shape Home (N) Turn 1 (L180) complete. Starting Leg 1 (30cm)..."); stopMotors(); prepareForwardMove(30.0); currentState = SHAPE_H_N_R1_LEG1; } break; }
        case SHAPE_H_N_R1_TURN2: { float angleTurned = abs(calculateAngleDifference(startYawForTurn, currentYaw)); if (angleTurned >= targetTurnAngle - TURN_ANGLE_THRESHOLD) { Serial.println("Shape Home (N) Turn 2 (L90) complete. Starting Leg 2 (105cm)..."); stopMotors(); prepareForwardMove(105.0); currentState = SHAPE_H_N_R1_LEG2; } break; }

        // --- Shape Home Return (Rover 1 - after M) States ---
        case SHAPE_H_M_R1_TURN1: { float angleTurned = abs(calculateAngleDifference(startYawForTurn, currentYaw)); if (angleTurned >= targetTurnAngle - (TURN_ANGLE_THRESHOLD * 1.5)) { Serial.println("Shape Home (M) Turn 1 (L180) complete. Starting Leg 1 (65cm)..."); stopMotors(); prepareForwardMove(45.0); currentState = SHAPE_H_M_R1_LEG1; } break; } // Corrected distance
        case SHAPE_H_M_R1_TURN2: { float angleTurned = abs(calculateAngleDifference(startYawForTurn, currentYaw)); if (angleTurned >= targetTurnAngle - TURN_ANGLE_THRESHOLD) { Serial.println("Shape Home (M) Turn 2 (L90) complete. Starting Leg 2 (110cm)..."); stopMotors(); prepareForwardMove(110.0); currentState = SHAPE_H_M_R1_LEG2; } break; }
        ...
        if (receivedMessage.msg_type == COMMAND_MOVEMENT) {
            ...
            if (currentState == SHAPE_N_R1_WAIT_HOME || currentState == SHAPE_M_R1_WAIT_HOME) {
                if (movCmd.command_type == 'h') {
                    ...
                    // Initiate 180 Left turn for return
                    ...
                    // Transition to the correct first turn state based on the shape
                    if (lastShapeCommand == 'n') { currentState = SHAPE_H_N_R1_TURN1; }
                    else if (lastShapeCommand == 'm') { currentState = SHAPE_H_M_R1_TURN1; }
                    else { Serial.println("Error: Unknown last shape command for home sequence!"); currentState = IDLE; } // Failsafe
                } else {
            ...
            switch(movCmd.command_type) {
                ...
                case 'n': { Serial.println("Action: Starting Shape N (105cm Fwd -> 90R -> 30cm Fwd)"); lastShapeCommand = 'n'; prepareForwardMove(105.0); currentState = SHAPE_N_R1_LEG1; break; }
                case 'm': { Serial.println("Action: Starting Shape M (110cm Fwd -> 90R -> 65cm Fwd)"); lastShapeCommand = 'm'; prepareForwardMove(110.0); currentState = SHAPE_M_R1_LEG1; break; } // Corrected distance comment
